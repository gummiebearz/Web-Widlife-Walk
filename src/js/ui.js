...
function toggleLoading(isLoading) {
  ...
  // clear the default text content of the button
  searchBtn.innerHTML = "";

  if (isLoading) {
    // Change button icon to hour class and add "Loading..."
    const hourGlass = document.createElement("i");
    hourGlass.className = "fas fa-hourglass-half";
    hourGlass.title = "hourglass";

    const searchTxt = document.createElement("span");
    searchTxt.innerText = "Loading...";

    // Add the inner content to a span
    searchBtn.append(hourGlass);
    searchBtn.append(searchTxt);
